export default {
  FORBIDDEN_KEY_UPDATE: {
    eng:
      "Error while trying to update the following forbidden keys: {{forbiddenProperties}}",
    ptBr:
      "Erro ao tentar atualizar as seguintes chaves: {{forbiddenProperties}}",
  },
  SERVER_BOOTSTRAP_ERROR: {
    eng: "Error while trying to bootstrap server on port {{port}}",
    ptBr: "Erro ao tentar inicializar o servidor na porta {{port}}",
  },
  SERVER_RUNNING: {
    eng:
      "Server is running on {{env}} | Port: {{port}} | Language: {{language}} | Timezone: {{timezone}} | Admin: {{adminEmail}}",
    ptBr:
      "Servidor rodando em {{env}} | Porta: {{port}} | Idioma: {{language}} | Timezone: {{timezone}} | Admin: {{adminEmail}}",
  },
  WAIT_MESSAGE: {
    eng: "Please wait...",
    ptBr: "Por favor, aguarde...",
  },
};
